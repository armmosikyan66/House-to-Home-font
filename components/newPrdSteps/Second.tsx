...
interface ImageUploaderProps extends StepProps {
    onChange?: (files: ImageFile[]) => void;
}

const Second: React.FC<ImageUploaderProps> = ({onChange, setPrdData}) => {
    const [imageFiles, setImageFiles] = useState<ImageFile[]>([]);
    const fileInputRef = useRef<HTMLInputElement>(null);

    useEffect(() => {
        setPrdData(prev => ({...prev, images: imageFiles.map(img => img.file)}));
    }, [imageFiles]);
                                            ...
                                            {imageFiles.length ? imageFiles.map((file) => (
                                                ...
                                                <span
                                                    ...
                                                    <p className="text-heading fs-22 lh-15 mb-4">Drag
                                                        and drop image
                                                        or</p>
                                                    <button className="btn btn-indigo px-7 mb-2"
                                                            type="button">
                                                        Browse file
                                                    </button>
                                                    <input
